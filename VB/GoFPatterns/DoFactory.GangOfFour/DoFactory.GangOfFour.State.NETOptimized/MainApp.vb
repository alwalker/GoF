...
Namespace DoFactory.GangOfFour.State.RealWorld
    ...
    Friend Class RedState
        ...
        Private Sub StateChangeCheck()
            If Me.Balance > upperLimit Then
                Me.Account.State = New SilverState(Me)
            End If
        End Sub
    End Class
    ...
    Friend Class SilverState
        ...
        Public Overrides Sub PayInterest()
            Me.Balance += interest * Me.Balance
            StateChangeCheck()
        End Sub

        Private Sub StateChangeCheck()
            If Me.Balance < lowerLimit Then
                Me.Account.State = New RedState(Me)
            ElseIf Me.Balance > upperLimit Then
                Me.Account.State = New GoldState(Me)
            End If
        End Sub
    End Class
    ...
    Friend Class GoldState
        ...
        Private Sub StateChangeCheck()
            If Me.Balance < 0.0 Then
                Me.Account.State = New RedState(Me)
            ElseIf Me.Balance < lowerLimit Then
                Me.Account.State = New SilverState(Me)
            End If
        End Sub
    End Class
